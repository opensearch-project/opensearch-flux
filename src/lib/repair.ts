...
export function parseNdjsonFile(ndjson: string) {
  return ndjson
    ...
    .filter((obj: any) => obj.type && obj.id);
}
...
export function remapDataSources(ndjson: string, sourceIds: Set<string>, targetId: string, targetDsTitle?: string): string {
  ...
  const remapped = lines.map((line) => {
    ...
    if (hasAnyDsRefs) {
      ...
      const alreadyHasDsRef = obj.references.some((r: any) => r.type === 'data-source');
      if (!alreadyHasDsRef) {
        obj.references.push({
          id: targetId,
          type: 'data-source',
 ...
 */
export function deepRemapDataSourceIds(value: unknown, sourceIds: Set<string>, targetId: string): any {
  if (typeof value === 'string') {
    try {
...
export function stripDashboardDatasetBlocks(ndjson: string): string {
  ...
  const stripped = lines.map((line) => {
    const obj = JSON.parse(line);
    if (!obj.type || !obj.id || obj.type !== 'dashboard') return line;
    ...
    try {
      ...
      if (parsed.query?.dataset) {
        ...
        log(`  Stripped dataset from dashboard ${obj.id}`);
      }
    } catch { /* skip */ }
...
export function disableMissingFieldFilters(ndjson: string): string {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      fieldsByPattern.set(obj.id, new Set(fields.map((f: any) => f.name)));
    } catch { /* skip */ }
  }
  ...
  const updated = lines.map((line) => {
    ...
    try {
      ...
      const indexRef = (obj.references || []).find(
        (ref: any) => ref.name === 'kibanaSavedObjectMeta.searchSourceJSON.index' && ref.type === 'index-pattern'
      );
      const fieldSet = indexRef ? fieldsByPattern.get(indexRef.id) : undefined;
      ...
      for (const filter of parsed.filter) {
        ...
        if (!fieldSet.has(key)) {
          ...
          changed = true;
          disabledCount++;
          log(`  Disabled filter "${key}" in ${obj.attributes?.title ?? obj.id} (field not in target index)`);
        }
      }
  ...
  log(`  Disabled ${disabledCount} filter(s) total.`);
  return updated.join('\n') + '\n';
}
